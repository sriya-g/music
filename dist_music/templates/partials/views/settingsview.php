<div class="view-container" id="music-user" ng-show="!loading">
	...
	<div ng-repeat="scrobbler in settings.scrobblers">
		...
		<div ng-if="scrobbler.identifier === 'listenbrainz'">
			<div>
				<p ng-show="!scrobbler.hasSession" translate>To connect with ListenBrainz, enter your private User Token. You can find it on your <a href="https://listenbrainz.org/settings/" target="_blank">ListenBrainz Settings</a> page.</p>
				<p ng-show="scrobbler.hasSession" translate>Your streams are currently being scrobbled to ListenBrainz. Click <strong>Disconnect</strong> to stop scrobbling.</p>
			</div>
			<div ng-show="!scrobbler.hasSession" style="margin-bottom: 10px;">
				<input type="password" ng-model="scrobbler.tokenInput" placeholder="User Token" style="width: 300px; max-width: 100%; margin-right: 10px;" />
				<button ng-click="scrobbler.submitListenBrainzToken(scrobbler.tokenInput)" ng-disabled="!scrobbler.tokenInput" translate>Connect</button>
			</div>
			<button ng-show="scrobbler.hasSession" ng-click="scrobbler.clearScrobbleSession()" translate>Disconnect</button>
		</div>
	</div>
